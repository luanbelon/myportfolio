...
async function ensureDefaultProjectsSeeded() {
  if (seeded) {
    return;
  }

  const countResult = await db.query('SELECT COUNT(*)::int AS count FROM projects');
...
module.exports = { ensureDefaultProjectsSeeded };
